...
public class HtmlUtil {
	...
	public static Map<String, Map<String, String>> parseHtmlTwo(InputStream in){
		...
		try {
			...
			for(Element dt : metasdts){
				...
				if(StringUtils.isNotBlank(webtapName) && map.size() > 0){
					resultMap.put(webtapName, map);
				}
			}
		} catch (Exception e) {
	...
	public static Map<String, List<Map>> parseElements(Element element,Map<String, List<Map>> resultMap){
		Map<String, Map> webtapMap = new HashMap<String, Map>();
				...
				for(Element a : dtas){
					if("a".equalsIgnoreCase(a.nodeName())){
			            ...
			            if(url.startsWith("http")){
			            	...
			            	webtapName=a.parent().parent().parent().child(0).text();
			            }
		...
		if(StringUtils.isNotBlank(webtapName)){
			webtapMap.put(webtapName, urlMap);
		}
